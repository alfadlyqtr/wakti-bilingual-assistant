...
serve(async (req) => {
  ...
  try {
    ...
    if (activeTrigger === 'image' || message.toLowerCase().includes('generate image') || message.toLowerCase().includes('أنشئ صورة')) {
      ...
      if (imageMode === 'background-removal' ? !Deno.env.get('KIE_API_KEY') : !RUNWARE_API_KEY) {
        return new Response(JSON.stringify({
          response: requestLanguage === 'ar'
            ? (imageMode === 'background-removal' ? 'أعتذر، خدمة إزالة الخلفية غير متاحة حالياً.' : 'أعتذر، خدمة إنشاء الصور غير متاحة حالياً.')
            : (imageMode === 'background-removal' ? 'I apologize, background removal service is not available.' : 'I apologize, image generation service is not available.'),
          error: true
        }), {
          status: 200,
          headers: { ...corsHeaders, 'Content-Type': 'application/json', 'Cache-Control': 'no-cache' }
