...
SIGNATURE FEATURES: the two to five details that give this specific room its character and that MUST survive any redesign — for example a raking or sloped glass facade, a timber-slatted ceiling, exposed diagonal steel bracing, a curved or angled wall, a double-height void, an arched colonnade, a feature staircase, exposed services, or a dramatic view out. Name each one plainly, say where it is, and then DESCRIBE ITS SHAPE AND ITS ORNAMENT IN ENOUGH DETAIL TO REDRAW IT FROM WORDS ALONE: how wide and how deep the element is, its profile or outline, and every piece of decoration on it — scrolls, spiral volutes, rosettes, circular medallions, carved panels, fluting, corbels, brackets, beading, and stepped or layered mouldings — saying how many there are and where each one sits. Do the same for any feature light fitting: its shape, its size and its material.
⚠️ An ornate element described only as "a decorative archway with ornate mouldings" is a FAILED description. The image model will redraw it as a plain flat band and the owner will not recognise their own room. Describe the decoration itself, piece by piece.
If the room genuinely has no distinctive feature, write "none".
