...
serve(async (req) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      const trial = await checkTrialAccess(supabaseService, user.id, 'voice_clone', 0);
      if (!trial.allowed) {
        return new Response(JSON.stringify({ success: false, ...buildTrialErrorPayload('voice_clone', trial) }), {
          status: 403,
