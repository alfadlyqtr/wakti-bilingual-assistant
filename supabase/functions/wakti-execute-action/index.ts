...
async function generateStylizedArt(prompt: string, inputImageUrl: string, userId: string, language: string) {
  try {
    console.log("Generating stylized art with Runware for prompt:", prompt, "and image:", inputImageUrl);
    ...
    if (!inputImageUrl || typeof inputImageUrl !== 'string') {
      throw new Error('Valid input image URL is required for stylized art generation');
    }
    ...
    if (response.ok) {
      ...
      if (stylizedResult && stylizedResult.imageURL) {
        ...
        // Save stylized art image to database
        try {
          await supabase
            .from('images')
            .insert({
          ...
          console.log("Could not save stylized art image to database:", dbError);
          // Continue anyway, the image was generated successfully
        }
