-- Extends accept_project_share to also copy project_ai_preferences,
-- project_images, and project_teams. These are project-scoped config/content
-- tables (not real end-customer data), so a full project transfer should
-- include them too.
--
-- Still intentionally NOT copied (unchanged from previous migration):
--   - Real end-customer/site-visitor data: project_bookings, project_carts,
--     project_chat_rooms, project_chat_messages, project_comments,
--     project_form_submissions, project_notifications, project_orders,
--     project_site_users, project_calendar_entries (auto-generated from
--     bookings, contains customer name/phone/email).
--   - The sender's own private AI history: project_generation_jobs (raw
--     prompts/errors) and user_helpful_memory (AI's private notes about the
--     sender). These belong to the sender's own usage, not the project's
...
begin
  ...
  -- Project-level AI build preferences (auto-approve migrations, preferred
  -- auth method/layout style, skip clarifying questions). Plain config, no
  -- customer data.
  insert into public.project_ai_preferences (project_id, user_id, auto_approve_migrations, preferred_auth_method, preferred_layout_style, skip_clarifying_questions)
  select v_new_project_id, auth.uid(), p.auto_approve_migrations, p.preferred_auth_method, p.preferred_layout_style, p.skip_clarifying_questions
  from public.project_ai_preferences p
  where p.project_id = v_share.source_project_id
  on conflict (project_id) do nothing;

  -- Project image metadata (url/alt/location/source records).
  insert into public.project_images (project_id, url, alt_text, location, file_path, source)
  select v_new_project_id, i.url, i.alt_text, i.location, i.file_path, i.source
  from public.project_images i
  where i.project_id = v_share.source_project_id;

  -- Project team entries (name + metadata).
  insert into public.project_teams (project_id, name, metadata)
  select v_new_project_id, t.name, t.metadata
  from public.project_teams t
  where t.project_id = v_share.source_project_id;
