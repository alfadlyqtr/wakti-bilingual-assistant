...
export default function Projects() {
  ...
  const [githubImportPat, setGithubImportPat] = useState('');
...
- Ensure smooth animations and transitions throughout`;
  ...
  const handleGitHubImport = async () => {
    ...
    if (projects.length >= MAX_PROJECTS) {
      toast.error(isRTL ? 'وصلت للحد الأقصى. احذف مشروعًا أولًا.' : 'Project limit reached. Delete a project first.');
      return;
    }
    ...
    try {
      ...
      // Step 1: Create a blank project first
      const repoSlug = githubImportUrl.split('github.com/').pop()?.replace('/', '-').replace(/[^a-z0-9-]/gi, '-').toLowerCase() || 'github-import';
      const { data: projectData, error: projectError } = await (supabase
        .from('projects' as any)
        .insert({
          user_id: session.user.id,
          name: repoSlug,
          slug: `${repoSlug}-${Date.now().toString(36)}`,
          description: `Imported from ${githubImportUrl}`,
          template_type: 'github-import',
          status: 'draft',
        })
        .select()
        .single() as any);
      ...
      // Step 2: Call github-import edge function
      toast.loading(isRTL ? 'جاري استيراد الملفات من GitHub...' : 'Importing files from GitHub...', { id: 'gh-import' });

      const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/github-import`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${session.access_token}`,
        },
        body: JSON.stringify({ repoUrl: githubImportUrl, projectId: projectData.id, ...(githubImportPat.trim() ? { githubPat: githubImportPat.trim() } : {}) }),
      });

      const result = await res.json();

      if (!res.ok || !result.success) {
        // Clean up the blank project on failure
        await (supabase as any).from('projects').delete().eq('id', projectData.id);
        toast.dismiss('gh-import');
        toast.error(result.error || (isRTL ? 'فشل الاستيراد' : 'Import failed'));
        return;
      }
      ...
      const outcome: string = result.outcome || 'direct';
      const framework: string = result.framework || 'React';

      if (outcome === 'rebuild') {
        toast(
          isRTL
            ? `تم تحليل ${result.fileCount} ملف · ${framework} · يحتوي على مكتبات متقدمة — سيعيد الذكاء الاصطناعي بناءه الآن ✨`
            : `Analyzed ${result.fileCount} files · ${framework} app detected · Uses advanced packages — AI is rebuilding it now ✨`,
          { duration: 6000, icon: '🔨' }
        );
      } else if (outcome === 'reference') {
        toast(
          isRTL
            ? `تم استيراد ${result.fileCount} ملف · ${framework} · المشروع محفوظ كمرجع — اطلب من الذكاء الاصطناعي إعادة بنائه`
            : `Imported ${result.fileCount} files · ${framework} detected · Saved as reference — ask the AI to rebuild it in Wakti`,
          { duration: 7000, icon: '📦' }
        );
      } else if (outcome === 'converted') {
        toast.success(
          isRTL
            ? `تم استيراد ${result.fileCount} ملف · ${framework} · تم التحويل تلقائيًا لـ Sandpack ✓`
            : `Imported ${result.fileCount} files · ${framework} auto-converted for Sandpack ✓`,
          { duration: 6000 }
        );
      } else {
        toast.success(
          isRTL
            ? `تم استيراد ${result.fileCount} ملف من GitHub ✓`
            : `Imported ${result.fileCount} files from GitHub ✓`,
          { duration: 5000 }
        );
      }
      ...
      // For rebuild: navigate with rebuild=true so ProjectDetail auto-submits to agent mode (which reads existing imported files)
      if (outcome === 'rebuild') {
        const rebuildPrompt = isRTL
          ? `أعد بناء هذا المشروع المستورد من GitHub (${result.repo}) ليعمل في المعاينة. استخدم الملفات الموجودة كمرجع كامل — نفس التصميم، نفس الصفحات، نفس الألوان والمحتوى والهيكل. اجعله يعمل باستخدام React + TailwindCSS فقط، بدون مكتبات تعتمد على Node.js. ممنوع استخدام import.meta أو import.meta.env أو process.env.`
          : `Rebuild this imported GitHub project (${result.repo}) as a fully working Sandpack-compatible React app. Study the existing imported files carefully and replicate the exact same design, pages, colors, content, and layout.

MANDATORY RULES — NEVER BREAK THESE:
- NEVER use import.meta, import.meta.env, or process.env anywhere
- NEVER use react-markdown, unified, rehype, remark, vfile, or any unified-ecosystem package
- NEVER use @supabase/supabase-js or any server-side package
- NEVER generate vite.config.ts, postcss.config.js, or tailwind.config.js files
- Use inline Tailwind CSS classes for all styling (CDN already loaded)
- Use only packages available in Sandpack: react, react-dom, react-router-dom, framer-motion, lucide-react
- If the original used react-markdown, render the markdown as plain styled HTML instead
- If the original had environment variables, replace them with empty strings or placeholder values
- Output a complete working app with all pages and components from the original`;
        navigate(`/projects/${projectData.id}?rebuild=true&prompt=${encodeURIComponent(result.repo)}&framework=${encodeURIComponent(framework)}`);
      } else {
        navigate(`/projects/${projectData.id}?imported=true&outcome=${outcome}&framework=${encodeURIComponent(framework)}`);
      }
            ...
            <div className="mt-5 mb-2">
              {!showGithubImport ? (
                <button
                  ...
                  className={cn(
                    "group w-full rounded-2xl border px-4 py-3 transition-all duration-200",
                    isDark
                      ? "border-white/10 bg-[linear-gradient(135deg,rgba(12,15,20,0.96)_0%,rgba(18,22,34,0.96)_50%,rgba(12,15,20,0.96)_100%)] hover:border-white/20 hover:shadow-[0_8px_30px_rgba(0,0,0,0.35),0_0_24px_rgba(59,130,246,0.12)]"
                      : "border-[#060541]/10 bg-[linear-gradient(135deg,rgba(252,254,253,0.98)_0%,rgba(244,247,255,0.98)_100%)] hover:border-[#060541]/20 hover:shadow-[0_12px_32px_rgba(6,5,65,0.08)]"
                  )}
                >
                    ...
                    <div className="flex items-center gap-3 min-w-0">
                      <div className={cn(
                        ...
                        isDark
                          ...
                          : "border-[#060541]/10 bg-[#060541]/5 text-[#060541]"
                      )}>
                      ...
                      <div className={cn("min-w-0 text-left", isRTL && "text-right")}>
                        <div className={cn(
                          ...
                          {isRTL ? 'استيراد مشروع من GitHub' : 'Import a project from GitHub'}
                        </div>
                        <div className={cn(
                          "text-xs mt-0.5",
                          isDark ? "text-white/55" : "text-[#060541]/55"
                        )}>
                          {isRTL ? 'استورد موقع أو قالب جاهز وابدأ التعديل عليه' : 'Bring in an existing site or template and keep building in Wakti'}
                        </div>
                      </div>
                    </div>
                    <div className={cn(
                      ...
                      isDark
                        ...
                        : "bg-[#060541]/6 text-[#060541]/75 group-hover:bg-[#060541]/10"
                    )}>
                      {isRTL ? 'GitHub ↗' : 'GitHub ↗'}
                    </div>
                  </div>
                ...
                <AnimatePresence>
                  <motion.div
                    ...
                    className={cn(
                      ...
                      isDark
                        ? "border-white/10 bg-[linear-gradient(135deg,rgba(12,15,20,0.98)_0%,rgba(18,22,34,0.98)_100%)] shadow-[0_10px_30px_rgba(0,0,0,0.35)]"
                        : "border-[#060541]/10 bg-white/95 shadow-[0_12px_32px_rgba(6,5,65,0.08)]"
                    )}
                  >
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <div className={cn("min-w-0", isRTL && "text-right")}>
                        <div className={cn(
                          ...
                          {isRTL ? 'ألصق رابط المستودع' : 'Paste your GitHub repository URL'}
                        </div>
                        <div className={cn(
                          "text-xs mt-1",
                          isDark ? "text-white/55" : "text-[#060541]/55"
                        )}>
                          {isRTL ? 'مثال: github.com/owner/repo أو github.com/owner/repo/tree/main' : 'Example: github.com/owner/repo or github.com/owner/repo/tree/main'}
                        </div>
                      </div>
                      <button
                        onClick={() => { setShowGithubImport(false); setGithubImportUrl(''); }}
                        className={cn(
                          "shrink-0 rounded-lg p-2 transition-colors",
                          isDark ? "text-white/45 hover:bg-white/6 hover:text-white/75" : "text-[#060541]/40 hover:bg-[#060541]/6 hover:text-[#060541]/70"
                        )}
                        title={isRTL ? 'إغلاق' : 'Cancel'}
                      >
                        <X className="h-4 w-4" />
                      </button>
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                      <div
                        className={cn(
                          "flex items-center gap-2 rounded-xl border px-3 py-2.5 transition-colors mb-2",
                          isDark ? "border-white/12 bg-white/5" : "border-[#060541]/10 bg-[#060541]/4"
                        )}
                      >
                        <LinkIcon className={cn("h-3.5 w-3.5 shrink-0",
                          isDark ? "text-white/45" : "text-[#060541]/40"
                        )} />
                        <input
                          type="url"
                          value={githubImportUrl}
                          onChange={(e) => setGithubImportUrl(e.target.value)}
                          onKeyDown={(e) => { if (e.key === 'Enter') handleGitHubImport(); if (e.key === 'Escape') { setShowGithubImport(false); setGithubImportUrl(''); setGithubImportPat(''); } }}
                          placeholder="https://github.com/owner/repo"
                          autoFocus
                          className={cn(
                            "flex-1 bg-transparent text-sm outline-none min-w-0",
                            isDark ? "text-white placeholder:text-white/28" : "text-[#060541] placeholder:text-[#060541]/30"
                          )}
                        />
                        {githubImportUrl && (
                          <a
                            ...
                            className={cn(
                              ...
                              isDark ? "text-white/35 hover:text-white/70" : "text-[#060541]/35 hover:text-[#060541]/70"
                            )}
                            tabIndex={-1}
                            title={isRTL ? 'فتح في GitHub' : 'Open on GitHub'}
                          >
                      ...
                      {/* Optional PAT for rate limit bypass */}
                      <div
                        className={cn(
                          "flex items-center gap-2 rounded-xl border px-3 py-2 mb-3",
                          isDark ? "border-white/8 bg-white/3" : "border-[#060541]/8 bg-[#060541]/3"
                        )}
                      >
                        <Github className={cn("h-3 w-3 shrink-0",
                          isDark ? "text-white/30" : "text-[#060541]/30"
                        )} />
                        <input
                          type="password"
                          value={githubImportPat}
                          onChange={(e) => setGithubImportPat(e.target.value)}
                          placeholder={isRTL ? 'GitHub PAT (اختياري — لتجنب حد المعدل)' : 'GitHub PAT (optional — avoids rate limit)'}
                          className={cn(
                            "flex-1 bg-transparent text-xs outline-none min-w-0",
                            isDark ? "text-white/70 placeholder:text-white/22" : "text-[#060541]/70 placeholder:text-[#060541]/25"
                          )}
                        />
                      </div>
                      ...
                      <Button
                        ...
                        disabled={importingFromGithub || !githubImportUrl.trim()}
                        className={cn(
                          "h-auto min-w-[120px] rounded-xl gap-2 px-4 py-3 text-white",
                          isDark
                            ? "bg-[linear-gradient(135deg,#060541_0%,hsl(260_70%_25%)_50%,#060541_100%)] hover:opacity-95 shadow-[0_8px_24px_rgba(6,5,65,0.35)]"
                            : "bg-[linear-gradient(135deg,#060541_0%,hsl(260_70%_25%)_50%,#060541_100%)] hover:opacity-95 shadow-[0_8px_24px_rgba(6,5,65,0.22)]"
                        )}
                      >
                        {importingFromGithub ? (
                          <Loader2 className="h-4 w-4 animate-spin" />
                        ...
                        <span className="text-xs sm:text-sm">{isRTL ? 'استيراد المشروع' : 'Import project'}</span>
                      </Button>
                    </div>
                  </motion.div>
                </AnimatePresence>
