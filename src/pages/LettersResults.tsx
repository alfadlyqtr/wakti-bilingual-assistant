...
export default function LettersResults() {
  ...
  React.useEffect(() => {
    ...
    async function load() {
      if (!code) return;
      const [{ data: rs }, { data: p }, { data: g }] = await Promise.all([
        supabase.from('letters_round_scores').select('user_id, total').eq('game_code', code),
      ...
      const totalsData: TotalRow[] = Array.from(map.entries()).map(([user_id, total]) => ({ user_id, total }));
  ...
  React.useEffect(() => {
    if (!code) return;
    const ch = supabase.channel(`letters:scores:${code}`)
