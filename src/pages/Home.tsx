  ...
  return <div className="mobile-container">
      <MobileHeader title="WAKTI" showUserMenu={false}>
        <div className="flex items-center gap-3">
          <Button variant="outline" size="sm" className="flex items-center gap-1 border-primary/20 hover:border-primary/40" onClick={() => navigate('/login')}>
            {t("login", language)}
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <section className="relative px-4 py-8 bg-gradient-to-br from-indigo-50 via-white to-purple-50 dark:from-indigo-950/30 dark:via-background dark:to-purple-950/30">
          ...
          <motion.div initial="hidden" animate="visible" variants={containerVariants} className="relative z-10 text-center">
            ...
            <motion.div variants={itemVariants} className="mb-8">
              <Button size="lg" className="w-full max-w-xs bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 hover:from-indigo-600 hover:via-purple-600 hover:to-pink-600 text-white font-semibold py-4 px-8 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300" onClick={() => navigate('/signup')}>
        ...
        once: true,
        margin: "-50px"
      }} variants={containerVariants} className="px-4 py-12 bg-gradient-to-b from-transparent to-slate-50/50 dark:to-slate-900/20">
        ...
        once: true,
        margin: "-50px"
      }} variants={containerVariants} className="px-4 py-12 bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 dark:from-indigo-950/20 dark:via-purple-950/20 dark:to-pink-950/20 mx-4 my-8 rounded-3xl backdrop-blur-sm border border-indigo-100 dark:border-indigo-800/30">
          <motion.div variants={itemVariants} className="text-center mb-8">
            ...
            <div className="inline-flex items-center gap-2 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm rounded-full p-1 border shadow-sm">
              <Button size="sm" variant={pricingPlan === "monthly" ? "default" : "ghost"} className="rounded-full text-sm px-6 py-2" onClick={() => setPricingPlan("monthly")}>
                {t("monthly", language)}
              </Button>
              <Button size="sm" variant={pricingPlan === "yearly" ? "default" : "ghost"} className="rounded-full text-sm px-6 py-2" onClick={() => setPricingPlan("yearly")}>
                {t("yearly", language)}
              </Button>
            </div>
          </motion.div>
          
          <motion.div key={pricingPlan} variants={itemVariants} className="bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm rounded-2xl p-8 shadow-xl border border-white/50 dark:border-slate-700/50 max-w-sm mx-auto">
            <div className="text-center mb-8">
              <div className="flex justify-center items-baseline gap-2 mb-4">
                <span className="text-4xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent">
                  {pricingPlan === "monthly" ? t("monthlyPrice", language) : t("yearlyPrice", language)}
                </span>
                {pricingPlan === "yearly" && <span className="text-sm bg-gradient-to-r from-green-500 to-emerald-500 text-white px-3 py-1 rounded-full font-medium shadow-sm">
                    {language === 'en' ? 'SAVE 17%' : 'وفر 17٪'}
                  </span>}
              </div>
              <p className="text-sm text-muted-foreground">
                {pricingPlan === "monthly" ? language === "en" ? "per month" : "شهرياً" : language === "en" ? "per year" : "سنوياً"}
              </p>
            </div>
            ...
            <Button className="w-full bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 hover:from-indigo-600 hover:via-purple-600 hover:to-pink-600 text-white font-semibold py-4 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300" onClick={() => navigate('/signup')}>
              <Star className="h-5 w-5 mr-2" />
              {t("createAccountNow", language)}
              <ArrowRight className="h-5 w-5 ml-2" />
            </Button>
