...
const SharedInbox = lazy(() => import('@/components/chatbot/SharedInbox'));
...
// MAIN COMPONENT
// ============================================
type WizardStep = 'list' | 'platform' | 'purpose' | 'dashboard' | 'builder' | 'designer' | 'ai-settings';
  ...
  const renderBotList = () => (
    <div className="container mx-auto p-3 max-w-4xl">
      ...
      <div>
              ...
              {loading ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {bots.map(bot => (
                    ...
                    >
                      <div className="h-[3px]" style={{ background: bot.primary_color }} />
                      <div className="p-5">
                        <div className="flex items-start justify-between mb-4">
                          <div className="flex items-center gap-3">
                            <div className="w-11 h-11 rounded-xl bg-[#060541]/6 dark:bg-white/10 flex items-center justify-center overflow-hidden">
                              {bot.avatar_url
                                ? <img src={bot.avatar_url} alt={bot.name} className="w-full h-full object-cover" />
                                : <Bot className="h-5 w-5 text-[#060541] dark:text-white" />}
                            </div>
                            <div>
          ...
          {/* ── INBOX TAB ── */}
          {innerTab === 'inbox' && (
            <motion.div key="inbox" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="h-[calc(100vh-200px)] min-h-[500px]">
              <Suspense fallback={<div className="flex items-center justify-center h-full"><Loader2 className="h-6 w-6 animate-spin text-muted-foreground" /></div>}>
                <SharedInbox bots={bots} isRTL={isRTL} />
              </Suspense>
            </motion.div>
          )}
  ...
  const renderPlatformSelect = () => (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 max-w-2xl mx-auto">
        ...
        <div className="relative rounded-2xl border-2 border-border/30 bg-white dark:bg-card overflow-hidden opacity-60 cursor-not-allowed">
          {/* Preview banner */}
          <div className="h-40 bg-gradient-to-br from-pink-50 to-purple-50 dark:from-pink-900/10 dark:to-purple-900/10 relative overflow-hidden flex items-center justify-center">
            ...
            <div className="absolute top-2.5 right-2.5 px-2 py-0.5 bg-amber-400/90 text-white text-[9px] font-bold rounded-full uppercase shadow">
              {isRTL ? 'قريباً' : 'Coming Soon'}
            </div>
          </div>
          {/* Info */}
    ...
    return (
      <div className="builder-fill" style={fixedStyle}>
        ...
        <div className="builder-header flex flex-col bg-white dark:bg-[#0c0f14]">
          ...
          <div className="h-[3px] w-full" style={{ background: activeBot.primary_color || '#060541' }} />
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center gap-3 mb-2">
          <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ background: accentColor + '20' }}>
        ...
        <div className="builder-fill">
          <Suspense fallback={<div className="flex items-center justify-center h-full"><Loader2 className="h-6 w-6 animate-spin text-muted-foreground" /></div>}>
            <ChatbotDesigner
              ...
              isRTL={isRTL}
              onBack={async () => {
                const fresh = await ChatbotService.getBot(activeBot.id).catch(() => null);
                if (fresh) { setActiveBot(fresh); setBots(prev => prev.map(b => b.id === fresh.id ? fresh : b)); }
                setStep('dashboard');
              }}
              onSave={async (updated) => {
                const fresh = await ChatbotService.getBot(updated.id).catch(() => updated);
                setActiveBot(fresh);
                setBots(prev => prev.map(b => b.id === fresh.id ? fresh : b));
                setStep('dashboard');
              }}
            />
          </Suspense>
