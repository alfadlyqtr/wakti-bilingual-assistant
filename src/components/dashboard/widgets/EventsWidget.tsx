...
import { Calendar as CalendarIconFull, Hand } from "lucide-react";
...
export const EventsWidget: React.FC<EventsWidgetProps> = ({ isLoading, events, language }) => {
  ...
  useEffect(() => {
    const fetchMaw3dEvents = async () => {
      try {
        console.log('EventsWidget: Fetching Maw3d events...');
        ...
        console.log('EventsWidget: Found today events:', todayEvents.length);
        setMaw3dEvents(todayEvents);
      } catch (error) {
        console.error('EventsWidget: Error fetching Maw3d events:', error);
        setMaw3dEvents([]);
      } finally {
  ...
  // Combine legacy events with Maw3d events (prioritize Maw3d)
  const allEvents = [...maw3dEvents, ...(events || [])];
  const hasEvents = allEvents && allEvents.length > 0;
  const isWidgetLoading = isLoading || maw3dLoading;

  return (
    <div className={`relative ${hasEvents ? 'p-4' : 'p-3'}`}>
      {/* Drag handle */}
      ...
      <div className="ml-10">
        ...
        {isWidgetLoading ? (
          ...
          <div className="text-center py-2">
            <CalendarIconFull className="mx-auto h-6 w-6 text-muted-foreground opacity-50 mb-1" />
            <p className="text-xs text-muted-foreground mb-2">{t("noEventsYet", language)}</p>
            <Button variant="outline" size="sm" onClick={() => navigate('/maw3d/create')}>
