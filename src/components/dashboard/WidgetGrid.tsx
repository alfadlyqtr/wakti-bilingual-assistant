...
const SortableWidget = ({ widget, isDragging }: { widget: WidgetType; isDragging: boolean }) => {
  ...
  return (
    <Card 
      ...
      className={`relative transition-all duration-200 w-full rounded-3xl overflow-hidden backdrop-blur-xl border border-white/10 bg-background/60 shadow-2xl ${
...
const StaticWidget = ({ widget }: { widget: WidgetType }) => {
  return (
    <Card className="relative transition-all duration-200 w-full rounded-3xl overflow-hidden backdrop-blur-xl border border-white/10 bg-background/60 shadow-2xl">
      {/* Glass reflection overlay */}
      <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-white/20 via-transparent to-transparent opacity-25 pointer-events-none" />
      <CardContent className="p-0 w-full relative z-10">
        {widget.component}
