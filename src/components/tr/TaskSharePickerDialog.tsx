...
import { getMutualTaskShareRecipients, sendTaskToContact, TaskShareRecipient } from '@/services/taskContactShareService';
import { Loader2, Search, Send, X } from 'lucide-react';
import { toast } from 'sonner';
import { Logo3D } from '@/components/Logo3D';

type ShareTaskTarget = {
  id: string;
  title: string;
...
interface TaskSharePickerDialogProps {
  isOpen: boolean;
  task: ShareTaskTarget | null;
  onClose: () => void;
  onSent?: () => void;
}

export function TaskSharePickerDialog({ isOpen, task, onClose, onSent }: TaskSharePickerDialogProps) {
  const { language } = useTheme();
  const isAr = language === 'ar';
  const [contacts, setContacts] = useState<TaskShareRecipient[]>([]);
  ...
  useEffect(() => {
    ...
    let active = true;
    setLoading(true);
    getMutualTaskShareRecipients()
      .then((rows) => {
        if (active) setContacts(rows);
      })
      .catch((error) => {
        console.error('[TaskSharePickerDialog] load error:', error);
  ...
  const handleSend = async (recipient: TaskShareRecipient) => {
    if (!task || sendingId) return;
    setSendingId(recipient.id);
    try {
      await sendTaskToContact({
        recipientId: recipient.id,
        recipientName: recipient.displayName,
        taskId: task.id,
      });
      toast.success(isAr ? `تم إرسال المهمة إلى ${recipient.displayName}` : `Task sent to ${recipient.displayName}`);
      onSent?.();
      onClose();
    } catch (error: any) {
      console.error('[TaskSharePickerDialog] send error:', error);
      toast.error((isAr ? 'فشل إرسال المهمة' : 'Failed to send task') + (error?.message ? `: ${error.message}` : ''));
    } finally {
      setSendingId(null);
    }
  };
      ...
      <div className="relative z-10 w-full max-w-md overflow-hidden rounded-[1.8rem] border border-fuchsia-400/20 bg-[linear-gradient(135deg,#0c0f14_0%,rgba(35,19,52,0.98)_45%,rgba(20,31,54,0.98)_100%)] text-white shadow-[0_20px_60px_rgba(0,0,0,0.55),0_0_40px_rgba(168,85,247,0.18)]">
        <div className="relative overflow-hidden border-b border-white/10 px-5 pb-4 pt-5">
          ...
          <div className="relative z-10 flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <h2 className="line-clamp-2 text-lg font-extrabold">{task.title}</h2>
              <p className="mt-1 text-sm text-white/75">
                {isAr ? 'أرسل هذه المهمة مباشرة إلى جهات اتصالك المشتركة في وقتي.' : 'Send this task directly to your mutual Wakti contacts.'}
              </p>
            </div>
          </div>
        </div>
        <div className="space-y-4 px-5 py-4">
          <div className="rounded-2xl border border-white/10 bg-white/5 p-3 text-sm text-white/80">
            {isAr ? 'سيظهر للطرف الآخر إشعار جميل داخل التطبيق. إذا قبل، ستظهر المهمة في تبويب المهام المشتركة لديه.' : 'The other person will receive an in-app popup. If they accept, the task will appear in their Shared Tasks tab.'}
          </div>
