...
import { ImageShareRecord } from '@/services/imageShareService';

interface ImageShareNotificationPopupProps {
  isOpen: boolean;
  share: ImageShareRecord | null;
  onClose: () => void;
  onAccept: () => void;
  onDecline: () => void;
...
export function ImageShareNotificationPopup({
  isOpen,
  share,
  onClose,
  onAccept,
  onDecline,
  isSubmitting = false,
}: ImageShareNotificationPopupProps) {
  ...
  const imagePrompt = share?.image_snapshot?.prompt || (isAr ? 'صورة' : 'Image');
  const imageUrl = share?.image_snapshot?.image_url || null;
              ...
              <CardContent className="p-0">
                <div className="relative overflow-hidden px-5 pt-5 pb-4">
                  ...
                  <div className="relative z-10 flex items-start gap-4">
                    <div className="h-20 w-20 overflow-hidden rounded-2xl border border-white/10 bg-white/5 shadow-[0_10px_30px_rgba(0,0,0,0.35)]">
                      {imageUrl ? (
                        <img src={imageUrl} alt={imagePrompt} className="h-full w-full object-cover" />
                      ) : (
                        <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-fuchsia-500/20 to-sky-500/20">
                          <ImageIcon className="h-8 w-8 text-fuchsia-200" />
                        </div>
                      )}
                    </div>
                    <div className="min-w-0 flex-1 pt-1">
                      <div className="mb-2 inline-flex items-center gap-1 rounded-full border border-fuchsia-400/25 bg-fuchsia-500/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-fuchsia-200">
                        ...
                        {isAr ? 'مشاركة صورة' : 'Image Share'}
                      </div>
                      <h2 className="truncate text-lg font-extrabold leading-tight text-white">
                        {senderName}
                      </h2>
                      <p className="mt-1 text-sm text-white/80">
                        {isAr ? 'يريد مشاركة هذه الصورة معك' : 'wants to share this image with you'}
                      </p>
                      <p className="mt-3 line-clamp-2 text-base font-semibold text-sky-100">
                        {imagePrompt}
                      </p>
                    </div>
                  </div>
                ...
                <div className="border-t border-white/10 bg-black/10 px-5 py-4">
                  <div className="mb-4 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm leading-relaxed text-white/80">
                    {isAr
                      ? 'إذا قبلت، سيتم حفظ هذه الصورة في الصور المحفوظة تلقائياً.'
                      : 'If you accept, this image will be saved to your Saved Images automatically.'}
                  </div>
