...
import { getMutualProjectShareRecipients, ProjectShareRecipient, sendProjectShare } from '@/services/projectShareService';
import { Code2, Search, Send, X, Loader2 } from 'lucide-react';
import { toast } from 'sonner';

type ShareProjectTarget = {
  id: string;
  title: string;
  thumbnailUrl: string | null;
};

interface ProjectSharePickerDialogProps {
  isOpen: boolean;
  project: ShareProjectTarget | null;
  onClose: () => void;
  onSent?: () => void;
}

export function ProjectSharePickerDialog({ isOpen, project, onClose, onSent }: ProjectSharePickerDialogProps) {
  const { language } = useTheme();
  const isAr = language === 'ar';
  const [contacts, setContacts] = useState<ProjectShareRecipient[]>([]);
  ...
  useEffect(() => {
    ...
    let active = true;
    setLoading(true);
    getMutualProjectShareRecipients()
      .then((rows) => {
        if (active) setContacts(rows);
      })
      .catch((error) => {
        console.error('[ProjectSharePickerDialog] load error:', error);
  ...
  const handleSend = async (recipient: ProjectShareRecipient) => {
    if (!project || sendingId) return;
    setSendingId(recipient.id);
    try {
      await sendProjectShare({ recipientId: recipient.id, projectId: project.id });
      toast.success(isAr ? `تم إرسال ${project.title} إلى ${recipient.displayName}` : `${project.title} sent to ${recipient.displayName}`);
      onSent?.();
      onClose();
    } catch (error: any) {
      ...
      <div className="relative z-10 w-full max-w-md overflow-hidden rounded-[1.8rem] border border-fuchsia-400/20 bg-[linear-gradient(135deg,#0c0f14_0%,rgba(35,19,52,0.98)_45%,rgba(20,31,54,0.98)_100%)] text-white shadow-[0_20px_60px_rgba(0,0,0,0.55),0_0_40px_rgba(168,85,247,0.18)]">
        <div className="relative overflow-hidden border-b border-white/10 px-5 pb-4 pt-5">
          ...
          <div className="relative z-10 flex items-center gap-3">
            <div className="h-16 w-16 overflow-hidden rounded-2xl border border-white/10 bg-white/5 shadow-[0_10px_30px_rgba(0,0,0,0.35)]">
              {project.thumbnailUrl ? (
                ...
                <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-fuchsia-500/20 to-sky-500/20">
                  <Code2 className="h-7 w-7 text-fuchsia-200" />
                </div>
              )}
            </div>
            <div className="min-w-0 flex-1">
              ...
              <h2 className="truncate text-lg font-extrabold">{project.title}</h2>
              <p className="mt-1 text-sm text-white/75">
                {isAr ? 'أرسل نسخة من هذا المشروع مباشرة إلى جهات اتصالك المشتركة.' : 'Send a copy of this project directly to your mutual Wakti contacts.'}
              </p>
            </div>
          </div>
        </div>
        <div className="space-y-4 px-5 py-4">
          <div className="rounded-2xl border border-white/10 bg-white/5 p-3 text-sm text-white/80">
            {isAr
              ? 'سيظهر للطرف الآخر إشعار جميل داخل التطبيق. إذا قبل، ستُنشأ نسخة جديدة ومستقلة في مشاريعه (بحد أقصى 3 مشاريع)، غير متصلة بأي GitHub.'
              : 'The other person will receive an in-app popup. If they accept, a new independent copy is created in their projects (max 3 projects), fully disconnected from any GitHub repo.'}
          </div>
