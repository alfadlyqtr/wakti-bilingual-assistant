...
const CustomFileSidebar = () => {
  ...
  return (
    <div className="w-64 h-full border-r border-white/10 bg-[#0f172a]/50 flex flex-col">
      <div className="p-4 border-b border-white/10">
        <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Project Files</h3>
      </div>
      <div className="flex-1 overflow-y-auto py-2">
        {Object.keys(files)
          ...
          >
            {fileName.endsWith('.js') || fileName.endsWith('.jsx') ? <FileCode className="w-4 h-4" /> : 
             fileName.endsWith('.css') ? <FileType className="w-4 h-4" /> : 
             <FileJson className="w-4 h-4" />}
            <span className="truncate">{fileName.replace('/', '')}</span>
            {activeFile === fileName && <CheckCircle2 className="w-3 h-3 ml-auto opacity-50" />}
          </button>
        ))}
      </div>
    </div>
...
<html lang="en">
  ...
  return (
    <div className="h-full w-full flex flex-col bg-[#0c0f14] border border-white/10 rounded-xl overflow-hidden shadow-2xl">
      {/* HEADER */}
      <div className="h-12 flex items-center justify-between px-4 border-b border-white/10 bg-[#0c0f14]">
        ...
        <div className="flex bg-white/5 rounded-lg p-1 border border-white/5">
          {(['preview', 'code'] as const).map((mode) => (
             <button 
               ...
               className={clsx(
                 "px-3 py-1.5 rounded-md text-xs font-medium transition-all capitalize flex items-center gap-2",
      ...
      <div className="flex-1 relative min-h-0">
        ...
        >
          <SandpackLayout style={{ height: '100%', width: '100%', border: 'none', backgroundColor: '#0c0f14' }}>
            ...
            {/* CENTER: CODE EDITOR */}
            {(viewMode === 'code') && (
              <div className="flex-1 h-full border-r border-white/10 overflow-hidden">
                <SandpackCodeEditor 
                  showTabs={false}
                  showLineNumbers 
                  showInlineErrors 
            ...
            {(viewMode === 'preview') && (
              <div className="flex-1 h-full relative bg-black">
                ...
                {elementSelectMode && (
                  <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-indigo-600 text-white px-4 py-2 rounded-full text-xs font-medium shadow-lg flex items-center gap-2 animate-pulse z-50">
