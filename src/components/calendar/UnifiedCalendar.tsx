...
export const UnifiedCalendar: React.FC = React.memo(() => {
  ...
  // Fetch calendar feed token from cached profile
  useEffect(() => {
    const fetchToken = async () => {
      if (!user?.id || !_calProfile) return;
      
      try {
        if (_calProfile.calendar_feed_token) {
          console.log('Calendar token loaded from cached profile');
          setCalendarFeedToken(_calProfile.calendar_feed_token);
        } else {
          console.error('No calendar token found in profile');
        }
      } catch (err) {
              ...
              <span className="text-[11px] text-muted-foreground whitespace-nowrap">
                {isSyncing
                  ...
                  : !nativeCalendarAvailable
                    ? (language === 'ar' ? 'غير متاح (السماح للتقويم)' : 'Not available (allow Calendar)')
                    : lastSyncAt
                      ? (language === 'ar'
                          ? `آخر مزامنة ${format(lastSyncAt, 'HH:mm')}`
                          : `Last ${format(lastSyncAt, 'HH:mm')}`)
