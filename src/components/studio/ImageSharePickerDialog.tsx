...
import { getMutualImageShareRecipients, ImageShareRecipient, sendImageShare } from '@/services/imageShareService';
import { ImageIcon, Loader2, Search, Send, X } from 'lucide-react';
...
interface ImageSharePickerDialogProps {
  isOpen: boolean;
  image: ShareImageTarget | null;
  onClose: () => void;
  onSent?: () => void;
}

export function ImageSharePickerDialog({ isOpen, image, onClose, onSent }: ImageSharePickerDialogProps) {
  const { language } = useTheme();
  const isAr = language === 'ar';
  const [contacts, setContacts] = useState<ImageShareRecipient[]>([]);
  ...
  useEffect(() => {
    ...
    let active = true;
    setLoading(true);
    getMutualImageShareRecipients()
      .then((rows) => {
        if (active) setContacts(rows);
      })
      .catch((error) => {
        console.error('[ImageSharePickerDialog] load error:', error);
  ...
  const handleSend = async (recipient: ImageShareRecipient) => {
    if (!image || sendingId) return;
    setSendingId(recipient.id);
    try {
      await sendImageShare({ recipientId: recipient.id, imageId: image.id });
      toast.success(isAr ? `تم إرسال الصورة إلى ${recipient.displayName}` : `Image sent to ${recipient.displayName}`);
      onSent?.();
      onClose();
    } catch (error: any) {
      console.error('[ImageSharePickerDialog] send error:', error);
      toast.error((isAr ? 'فشل إرسال الصورة' : 'Failed to send image') + (error?.message ? `: ${error.message}` : ''));
    } finally {
      setSendingId(null);
    }
  };
      ...
      <div className="relative z-10 w-full max-w-md overflow-hidden rounded-[1.8rem] border border-fuchsia-400/20 bg-[linear-gradient(135deg,#0c0f14_0%,rgba(35,19,52,0.98)_45%,rgba(20,31,54,0.98)_100%)] text-white shadow-[0_20px_60px_rgba(0,0,0,0.55),0_0_40px_rgba(168,85,247,0.18)]">
        <div className="relative overflow-hidden border-b border-white/10 px-5 pb-4 pt-5">
          ...
          <div className="relative z-10 flex items-center gap-3">
            <div className="h-16 w-16 overflow-hidden rounded-2xl border border-white/10 bg-white/5 shadow-[0_10px_30px_rgba(0,0,0,0.35)]">
              {image.imageUrl ? (
                ...
                <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-fuchsia-500/20 to-sky-500/20">
                  <ImageIcon className="h-7 w-7 text-fuchsia-200" />
                </div>
              )}
            </div>
            <div className="min-w-0 flex-1">
              ...
              <h2 className="line-clamp-2 text-lg font-extrabold">{image.title}</h2>
              <p className="mt-1 text-sm text-white/75">
                {isAr ? 'أرسل هذه الصورة مباشرة إلى جهات اتصالك المشتركة.' : 'Send this image directly to your mutual Wakti contacts.'}
              </p>
            </div>
          </div>
        </div>
        <div className="space-y-4 px-5 py-4">
          <div className="rounded-2xl border border-white/10 bg-white/5 p-3 text-sm text-white/80">
            {isAr ? 'سيظهر للطرف الآخر إشعار جميل داخل التطبيق. إذا قبل، سيتم حفظ الصورة في تبويب الصور المحفوظة.' : 'The other person will receive an in-app popup. If they accept, the image will be saved in their Saved Images tab.'}
          </div>
