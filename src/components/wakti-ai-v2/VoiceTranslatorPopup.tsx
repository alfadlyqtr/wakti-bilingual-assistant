...
export function VoiceTranslatorPopup({ open, onOpenChange }: VoiceTranslatorPopupProps) {
  ...
  // OPTIMIZED: Handle successful translation with INSTANT auto-TTS and auto-play
  const handleOptimizedTranslationSuccess = useCallback(async (data: any) => {
    ...
    const newTranslation: TranslationItem = {
      ...
      timestamp: new Date(),
      audioContent: data.autoGeneratedTTS?.audioContent // Pre-generated TTS
    };
    addToHistory(newTranslation);
    
    // INSTANT: Cache and preload the auto-generated TTS
    if (data.autoGeneratedTTS?.audioContent) {
      ...
      // INSTANT: Preload for zero-delay playback
      audioManager.preloadAudio(cacheKey, data.autoGeneratedTTS.audioContent);
      
      console.log('🔊 INSTANT: Auto-generated TTS cached and preloaded');
    }
    
    // INSTANT AUTO-PLAY: If toggle is enabled, play immediately
    if (playbackEnabled && data.autoGeneratedTTS?.audioContent) {
      console.log('🔊 INSTANT AUTO-PLAY: Playing translation immediately');
      try {
        setIsPlaying(true);
        const cacheKey = `${data.translatedText}_${selectedLanguage}`;
        await audioManager.playAudioInstantly(data.autoGeneratedTTS.audioContent, cacheKey);
        setIsPlaying(false);
        console.log('🔊 INSTANT AUTO-PLAY: Completed successfully');
      } catch (playError) {
        console.error('🔊 INSTANT AUTO-PLAY: Failed:', playError);
        setIsPlaying(false);
      }
    }
  ...
  // OPTIMIZED: Lightning-fast TTS playback with instant cache lookup
  const playTranslatedTextInstantly = useCallback(async (text: string) => {
    console.log('🔊 INSTANT PLAY: User clicked play button');
    
    if (audioManager.isAudioPlaying()) {
      console.log('🔊 INSTANT PLAY: Stopping current audio');
      audioManager.stopCurrentAudio();
      setIsPlaying(false);
      return;
    }
    ...
    try {
      ...
      // INSTANT: Try cache first for zero-delay playback
      if (audioCache[cacheKey] && audioCache[cacheKey].data) {
        console.log('🔊 INSTANT PLAY: Playing from cache instantly');
        await audioManager.playAudioInstantly(audioCache[cacheKey].data, cacheKey);
        setIsPlaying(false);
        return;
      }
      
      console.log('🔊 INSTANT PLAY: Generating TTS via optimized unified-ai-brain');
      setIsGeneratingAudio(true);
      
      // DIRECT: Call optimized TTS function
      const { data, error } = await supabase.functions.invoke('unified-ai-brain', {
        body: JSON.stringify({
          text: text,
          voice: 'alloy',
      ...
      if (error) {
        console.error('🔊 INSTANT PLAY: TTS error:', error);
        throw new Error(error.message || 'TTS service error');
      }

      if (data?.audioContent) {
        console.log('🔊 INSTANT PLAY: TTS generated, playing instantly');
        
        // INSTANT: Cache for future zero-delay playback
        const newCache = { 
          ...audioCache, 
          [cacheKey]: {
        ...
        // INSTANT: Preload and play immediately
        audioManager.preloadAudio(cacheKey, data.audioContent);
        await audioManager.playAudioInstantly(data.audioContent, cacheKey);
        console.log('🔊 INSTANT PLAY: Completed successfully');
      } else {
        throw new Error('No audio content received from optimized TTS');
      }
    } catch (error) {
      console.error('🔊 INSTANT PLAY: Error:', error);
      
      const errorMessage = language === 'ar' ? 'فشل في تشغيل الصوت' : 'Audio playback failed. Please try again.';
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center space-x-2">
              ...
              <Label htmlFor="playback" className="text-sm font-medium">
                {language === 'ar' ? 'تشغيل فوري' : 'Instant Play'}
              </Label>
          ...
          <div className="text-center space-y-4">
            ...
            {isProcessing && (
              <div className="flex items-center justify-center gap-2 text-blue-600">
                ...
                <span className="text-sm font-medium">
                  {language === 'ar' ? 'معالجة محسنة...' : 'Processing with instant TTS...'}
                </span>
              </div>
            )}
            ...
            <p className="text-xs text-muted-foreground">
              {language === 'ar' 
                ? 'اضغط للتسجيل الفوري (حتى 15 ثانية)'
            ...
            <div className="space-y-3">
              ...
              {playbackEnabled && (
                <div className="text-center text-xs text-green-600 dark:text-green-400">
                  {language === 'ar' 
                    ? '✨ تم التشغيل الفوري - يمكنك الضغط على تشغيل للإعادة'
                    : '✨ Played instantly - click Play to replay'
                  }
                </div>
              )}
            </div>
