    ...
    return (
      <div className="space-y-4 max-w-sm mx-auto">
        <p className="text-center text-sm text-slate-600 dark:text-slate-400">
          {isAr ? 'العب ضد صديق عبر رمز قصير' : 'Play vs a friend with a short code'}
        </p>
  ...
  if (mode === 'create') {
    return (
      <div className="space-y-4 max-w-sm mx-auto">
        ...
        <div>
          <Label className="text-xs mb-1 block">{isAr ? 'اختر رمزك' : 'Choose your symbol'}</Label>
          <div className="flex gap-2">
            <Button
              variant={symbol === 'X' ? 'default' : 'outline'}
              onClick={() => setSymbol('X')}
              className="flex-1 min-h-[44px] text-lg"
            >X</Button>
            <Button
              variant={symbol === 'O' ? 'default' : 'outline'}
              onClick={() => setSymbol('O')}
              className="flex-1 min-h-[44px] text-lg"
            >O</Button>
          </div>
