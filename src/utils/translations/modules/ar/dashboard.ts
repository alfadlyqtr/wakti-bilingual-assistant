
export const dashboardAr = {
  dailyQuote: "اقتباس يومي",
  nothingScheduled: "لا توجد مواعيد مجدولة",
  event: "حدث",
  calendar_open: "فتح التقويم",
  events_view_all: "عرض جميع الأحداث",
  tasksReminders: "المهام والتذكيرات",
  maw3dEvents: "أحداث Maw3d",
  dragMode: "وضع السحب",
  dragModeActivated: "تم تفعيل وضع السحب",
  dragModeDeactivated: "تم إلغاء تفعيل وضع السحب",
  noEventsYet: "لا توجد أحداث بعد",
  createEvent: "إنشاء حدث",
  organizeWidgets: "تنظيم الأدوات",
  exitDragMode: "إيقاف السحب",
  welcome: "مرحبًا",
  moodTrend: "اتجاه المزاج",
  journal: "المذكرة",
  free_trial_welcome_message: "استمتع بتطبيق وقتي AI بجميع ميزاته لمدة 30 دقيقة! بعد انتهاء صلاحية الوصول، يمكنك الاشتراك لفتح الاستخدام غير المحدود والحصول على فترة تجريبية مجانية لمدة 3 أيام في خطتنا الشهرية ."
};
