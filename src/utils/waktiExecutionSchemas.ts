...
export const WAKTI_EXECUTION_SCHEMAS: Record<WaktiCapabilityId, WaktiExecutionSchema> = {
  ...
  image_studio: {
    ...
    actions: [
      ...
      action('save_or_share_image', 'Save or share image', 'حفظ أو مشاركة الصورة', 'Save a finished image or prepare it for sharing.', 'حفظ صورة مكتملة أو تجهيزها للمشاركة.', 'prepare', 'review', '/music?operatorTarget=image', 'Image Studio / Result', 'استوديو الصور / النتيجة', [field('image', 'Generated image', 'الصورة المنشأة', 'file', true, 'Choose the generated image to save or share.', 'اختر الصورة المنشأة المطلوب حفظها أو مشاركتها.')], { adapter: 'image_result_handoff' }),
    ],
  },
  music_studio: {
  ...
  video_studio: {
    ...
    actions: [
      action('create_text_video', 'Prepare text-to-video', 'تجهيز فيديو من نص', 'Open the AI Videomaker text-to-video workflow and validate the real video description.', 'فتح تدفق الفيديو من النص والتحقق من وصف الفيديو الحقيقي.', 'prepare', 'review', '/music?studioTab=video', 'Video Studio / AI Videomaker', 'استوديو الفيديو / صانع الفيديو بالذكاء', [field('prompt', 'Video description', 'وصف الفيديو', 'long_text', true, 'Describe subject, environment, camera movement, style, and desired motion.', 'صف الموضوع والبيئة وحركة الكاميرا والنمط والحركة المطلوبة.'), field('duration', 'Duration', 'المدة', 'choice', false, 'Choose a duration when stated.', 'اختر مدة عند ذكرها.'), field('resolution', 'Resolution', 'الدقة', 'choice', false, 'Choose 480p, 720p, or 1080p when stated.', 'اختر 480p أو 720p أو 1080p عند ذكرها.', { allowedValues: ['480p', '720p', '1080p'] })], { adapter: 'video_text_to_video', backend: ['AI Videomaker'] }),
  ...
  voice_studio: {
    ...
    actions: [
      action('generate_speech', 'Generate speech', 'إنشاء صوت', 'Turn supplied text into spoken audio.', 'تحويل النص المرفق إلى صوت منطوق.', 'run', 'review', '/tools/voice-studio?tab=tts', 'Voice Studio / Text to Speech', 'استوديو الصوت / تحويل النص إلى كلام', [field('text', 'Text to speak', 'النص المطلوب نطقه', 'long_text', true, 'Provide the exact text to speak.', 'أضف النص الدقيق المطلوب نطقه.'), field('voice', 'Voice', 'الصوت', 'choice', false, 'Choose a voice when one is specified.', 'اختر صوتاً عندما يحدد المستخدم صوتاً.')], { adapter: 'voice_tts', backend: ['generate-speech'], preconditionsEn: ['The person must be signed in.', 'A selected custom voice must be ready.'], preconditionsAr: ['يجب تسجيل دخول المستخدم.', 'يجب أن يكون الصوت المخصص المختار جاهزاً.'] }),
  ...
  settings: {
    ...
    actions: [
      ...
      action('change_language', 'Change language', 'تغيير اللغة', 'Open Settings ready to review the app language.', 'فتح الإعدادات جاهزة لمراجعة لغة التطبيق.', 'guide', 'required', '/settings', 'Settings', 'الإعدادات', [field('language', 'Language', 'اللغة', 'choice', true, 'Choose the app language.', 'اختر لغة التطبيق.')]),
...
export function buildWaktiExecutionKnowledgeManifest(language: 'ar' | 'en') {
  return Object.values(WAKTI_EXECUTION_SCHEMAS)
    .flatMap((schema) => schema.actions.map((item) => {
      const fields = item.fields.map((fieldSchema) => `${fieldSchema.key}${fieldSchema.required ? '*' : ''}${fieldSchema.requiredWhen ? `(${fieldSchema.requiredWhen})` : ''}`).join(',') || 'none';
